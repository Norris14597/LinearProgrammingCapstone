        ...
        for (Course c : courseList) {
            ...
            for (Section s : c.getCourseSections()) {
                ...
                for (Room r : roomList) {
                    ...
                    double Hij = LinearProgramming.happinessRoomVal(r, c); //type and seating
                    ...
                    if (Hij >= 1) {
                        ...
                        if (c.getCredits() == 1 && professorScheduleType.equals("MWF")) {
                            ...
                            for (int i = 0; i < p.getAvailableDayTimes().size(); i++) {
                                ...
                                DayTimes d = p.getAvailableDayTimes().get(i); //individual day within loop
                                ...
                                for (int j = 0; j < d.getDayTimes().size(); j++) {
                                   //professor and room available during hour of day
                                    boolean isHourAvailableForRoom =
                                            LinearProgramming.isRoomTimesAvailable(j, roomDays, c.getCredits(), d.getDay());
                                    boolean isHourAvailableForProfessor =
                                            LinearProgramming.isProfessorTimesAvailable(j, professorDays, c.getCredits());

                                    //professor/room available for one hour + 15 minutes before and after
                                    if (isHourAvailableForProfessor && isHourAvailableForRoom) {
                                        ...
                                        if (sectionSum > bestSectionSum) {
                                            ...
                                            bestSectionSum = sectionSum; //new best section sum
                                            bestMWF = LinearProgramming.clearAllTimeSlots(bestMWF); //clear best available time
                                            bestMWF = LinearProgramming.assignTimeSlots(bestMWF, j, c.getCredits(), d.getDay()); //assign new times
                                            bestRoom = r; //set as best room
                                            isSectionAssigned = true; //set section has been assigned
                                        }
                                    }
                                } //end times
                            } //end days
                        ...
                        if (c.getCredits() == 1 && professorScheduleType.equals("TR")) {
                            ...
                            for (int i = 0; i < p.getAvailableDayTimes().size(); i++) {
                                ...
                                //"timelength" in a day 15 minutes
                                for (int j = 0; j < d.getDayTimes().size(); j++) {
                        ...
                        if (c.getCredits() == 3 && professorScheduleType.equals("MWF")) {
                            ...
                            for (int j = 0; j < professorDays.get(0).getDayTimes().size(); j++) {
                                ...
                                boolean isHourAvailableForProfessor =
                                        LinearProgramming.isProfessorTimesAvailable(j, professorDays, c.getCredits());
                                boolean isHourAvailableForRoom =
                                        LinearProgramming.isRoomTimesAvailable(j, roomDays, c.getCredits(), professorDays.get(0).getDay());
                ...
                if (isSectionAssigned) {
                    //    copy to bestTimes either MWF or TR schedule depending on professor
                    ...
                    p.getSectionsTaught().add(s); //add section to professor's teaching list

                    bestRoom.addDayTimes(bestTimes); //assign best times to the best room fit
                    filledSections.add(s);



                    //********************* CHECK THIS AND REMOVE ONCE FINISHED DEBUGGING
